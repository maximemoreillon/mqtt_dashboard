<template>
  <v-card outlined>
    <v-toolbar flat>
      ...
      <v-btn v-if="!device.connected" icon disabled>
      ...
      <v-menu offset-y>
        ...
        <v-list dense>
          <v-list-item>
            <v-list-item-content>
              <v-btn text @click="clear_retained_messages()">
    ...
    <v-expansion-panels flat>
      <v-expansion-panel>
        <v-expansion-panel-header> Device details </v-expansion-panel-header>
          ...
          <v-list>
            ...
            <v-list-item two-line>
              <v-list-item-content>
                ...
                <v-list-item-title>
                  {{ device.firmware_version || device.version || "unknown" }}
                </v-list-item-title>
              </v-list-item-content>
            </v-list-item>

            <!-- Special addition for IR controllers -->
            <v-list-item two-line v-if="device.type === 'ircontroller'">
              <v-list-item-content>
                <v-list-item-subtitle>IR signal recording</v-list-item-subtitle>
                <v-row align="center">
                  <v-col cols="auto">
                    <v-btn @click="record('on')"> Record ON </v-btn>
                  </v-col>
                  <v-col cols="auto">
                    <v-btn @click="record('off')"> Record OFF </v-btn>
                  </v-col>
                  <v-spacer />
                </v-row>
              </v-list-item-content>
...
export default {
  ...
  methods: {
    turn_on() {
      if (this.device.json)
        mqtt_client.publish(this.command_topic, JSON.stringify({ state: "on" }))
